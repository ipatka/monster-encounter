...
contract LootPersonality is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function getRaces(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "RACES", races);
    }
    ...
    function getPhysique(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "PHYSIQUE", physique);
    }
    ...
    function getVirtue(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "VIRTUE", virtue);
    }
    
    function getVice(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "VICE", vice);
    }
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        parts[7] = getBackground(tokenId);

        parts[8] = '</text><text x="10" y="100" class="base">';

        parts[9] = getVirtue(tokenId);

        parts[10] = '</text><text x="10" y="120" class="base">';

        parts[11] = getVice(tokenId);
    ...
    constructor() ERC721("Loot Personality", "LOOTPERSONALITY") Ownable() {}
}
